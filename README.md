...
El diagrama de navegación adjunto modela la arquitectura de enrutamiento y el control de acceso lógico basado en roles (RBAC) estructurado e implementado en la plataforma B2B EcoMóstoles. Para garantizar una compresión inequívoca de las fronteras de red durante la auditoría técnica, el mapa topológico emplea una codificación semántica fundamentada en el color de sus vectores (flechas). Esta leyenda visual sectoriza la aplicación en tres grandes dominios de seguridad, delineando los flujos de interacción del cliente según el estado de su sesión y el nivel de escalada de sus privilegios corporativos.

#### 2. Flujo Público (Flechas Amarillas - Navegación Anónima)

La red de tractos representada por las flechas amarillas ilustra el ecosistema de tráfico público, concebido sin restricciones lógicas (ausencia de guardias de autorización). Este flujo actúa macroestructuralmente como embudo de captación (_funnel_) para cualquier entidad anónima. La interacción gravita en torno al nodo de entrada o _Landing Page_ (`index.html`), desde el cual nacen las bifurcaciones nativas hacia los controladores de identidad: el panel de inicio de sesión (`login.html`) y el bloque de incorporación al sistema (`registro.html`). Las directrices amarillas evidencian el tránsito cíclico e irrestricto entre estos tres archivos, permitiendo a los usuarios retroceder e iterar libremente antes de desencadenar el cruce de la pasarela de autenticación.

#### 3. Flujo de Empresa Registrada (Flechas Azules - Core Operativo)

Tras rebasar la barrera de validación, las flechas azules trazan el núcleo de negocio (_Core Operativo_) blindado de la plataforma B2B, restringido en su totalidad a cuentas corporativas validadas. La topología arquitectónica de este clúster opera bajo un patrón radial (_Hub and Spoke_). El panel de mandos masivo o **Dashboard** asume el rol de nodo maestro y matriz de operaciones. Desde él se originan múltiples rutas vectoriales azules que otorgan a la empresa operabilidad completa e ininterrumpida sobre los subsistemas CRUD (Crear, Editar, Listar y ver Detalle) asociados a las tres entidades maestras del modelo ontológico: las **Ofertas**, las **Demandas** y los **Acuerdos transaccionales**. Del mismo modo, este dominio acota las derivaciones transversales que interconectan el nodo central con periféricos críticos, tales como el gestor centralizado de mensajería y el control del perfil empresarial.

#### 4. Flujo de Administración (Flechas Verdes - Supervisión Master)

La topología cartografiada por las flechas verdes define las rutas de máximo nivel de aislamiento, encapsuladas y asignadas única y exclusivamente para las autoridades de rol Administrador. Este canal dirige el tráfico de forma unívoca hacia su propio ecosistema de _back-office_ (`admin_panel.html`), un recuadro hermético desde donde la administración general despliega submódulos tácticos orientados al mantenimiento integral: monitorización de reportes métricos, censado y revocación de usuarios, y configuración global de las variables de la plataforma.

> **⚠️ Nota Arquitectónica Crítica (Herencia de Propagación Lógica):**
> A nivel ingenieril, la implementación del rol Administrador soporta un paradigma de inherencia o "herencia vertical de permisos". Esto dicta formalmente que, aunque el coto natural del mantenedor venga codificado por los flujos de rutas verdes, su token de autorización porta legitimidad de acceso global omnipotente. Consecuentemente, el administrador cuenta con la capacidad de atravesar transversalmente la plataforma y discurrir, de forma lícita, por la totalidad del entramado operativo de los usuarios ordinarios (esto es, por cualquier ruta marcada por las **flechas azules**), posibilitando una auditoría en tiempo real de las operaciones de cada delegación.
...
|                                                            12                                                            |                                                                                    [Add authentication and form handling scripts; implement particle effects for login and registration pages                                                                                    |
| ](https://github.com/CodeURJC-SSDD-2025-26/practica-ssdd-2025-26-grupo-10/tree/95564bd80a017502fabf9a5ab436a9af6de199b8) |                                                          [crear_acuerdo.html](https://github.com/CodeURJC-SSDD-2025-26/practica-ssdd-2025-26-grupo-10/blob/95564bd80a017502fabf9a5ab436a9af6de199b8/crear_acuerdo.html)                                                          |
