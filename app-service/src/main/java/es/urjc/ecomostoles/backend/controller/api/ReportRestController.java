...
public class ReportRestController {
    ...
    public ResponseEntity<CompanyImpactDTO> getMyImpact(Principal principal) {
        if (principal == null) {
            log.warn("[API] GET /reports/company/me — Unauthorized access attempt");
            throw new org.springframework.web.server.ResponseStatusException(
                    org.springframework.http.HttpStatus.UNAUTHORIZED);
        }

        log.debug("[API] GET /reports/company/me — Calculating impact for: {}", principal.getName());

        Company company = companyService.findByEmail(principal.getName())
                .orElseThrow(() -> new org.springframework.web.server.ResponseStatusException(
                        org.springframework.http.HttpStatus.NOT_FOUND, "Company not found: " + principal.getName()));
        ...
        co2Saved = Math.round(co2Saved * 100.0) / 100.0;
        materialsManaged = Math.round(materialsManaged * 100.0) / 100.0;

        log.info("[API] GET /reports/company/me — Result for {}: {} Kg CO2, {} Kg material",
                company.getCommercialName(), co2Saved, materialsManaged);
