 ...
 *
 * <p>
 * Base path: {@code /api/v1/agreements}
 * </p>
 */
@RestController
...
public class AgreementRestController {
         ...
         *         {@link AgreementDTO}.
         */
        @Operation(summary = "List agreements (Paginated)", description = "Returns a paginated object containing agreements. Metadata includes totalElements and totalPages.")
        @ApiResponses({
        ...
        /**
         * Returns a non-paginated list of agreements where the authenticated company
         * is either the origin or the destination.
         */
        @Operation(summary = "List MY agreements", description = "Returns a flat list of agreements where the authenticated user is involved.")
        @GetMapping("/me")
        public ResponseEntity<java.util.List<AgreementDTO>> getMyAgreements(Principal principal) {

                if (principal == null)
                        throw new ResponseStatusException(HttpStatus.UNAUTHORIZED);

                log.debug("[API] GET /api/v1/agreements/me — User: {}", principal.getName());

                Company company = agreementService.findCompanyByEmail(principal.getName());

                java.util.List<Agreement> agreements = agreementService.getByCompany(company);

                return ResponseEntity.ok(agreements.stream()
                                .map(agreementMapper::toDto)
                                .toList());
        }
        ...
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Agreement found", content = @Content(schema = @Schema(implementation = AgreementDTO.class))),
        ...
        public ResponseEntity<AgreementDTO> getAgreementById(@PathVariable Long id, Principal principal) {
                ...
                // IDOR Protection: Verify that the principal is part of the agreement or an
                // ADMIN
        ...
        @ApiResponses({
                        @ApiResponse(responseCode = "201", description = "Agreement created successfully", content = @Content(schema = @Schema(implementation = AgreementDTO.class))),
        ...
        public ResponseEntity<AgreementDTO> createAgreement(@Valid @RequestBody AgreementDTO agreementDTO) {
                ...
                Long destinationCompanyId = (agreementDTO.destinationCompany() != null)
                                ? agreementDTO.destinationCompany().getId()
                                : null;

                agreementService.registerNewAgreement(agreement, userEmail, agreementDTO.offerId(),
        ...
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Agreement updated successfully", content = @Content(schema = @Schema(implementation = AgreementDTO.class))),
                        @ApiResponse(responseCode = "400", description = "Validation failed", content = @Content),
                        @ApiResponse(responseCode = "404", description = "Agreement not found", content = @Content)
        })
        @PutMapping("/{id}")
        public ResponseEntity<AgreementDTO> updateAgreement(
                ...
                if (principal == null || (!isOrigin && !isDestination && !isAdmin)) {
                        ...
                        throw new ResponseStatusException(
                                        HttpStatus.FORBIDDEN, "You are not authorized to modify this agreement");
                }

                // Delegating entirely to the updateAgreement method in the service which
        ...
        public ResponseEntity<AgreementDTO> updateAgreementStatus(
                ...
                if (principal == null || (!isOrigin && !isDestination && !isAdmin)) {
                        log.warn("[SECURITY] IDOR attempt blocked: User {} tried to update status for agreement {}",
                                        principal != null ? principal.getName() : "anonymous", id);
                        throw new ResponseStatusException(
                                        HttpStatus.FORBIDDEN, "You are not authorized to modify this agreement status");
                }
        ...
        public ResponseEntity<Void> deleteAgreement(@PathVariable Long id, Principal principal) {
                ...
                // IDOR Protection: Verify that the principal is part of the agreement (origin
                // or destination) OR an ADMIN
                boolean isOrigin = agreement.getOriginCompany() != null
                                && agreement.getOriginCompany().getContactEmail().equals(principal.getName());
                boolean isDestination = agreement.getDestinationCompany() != null
                                && agreement.getDestinationCompany().getContactEmail().equals(principal.getName());
                ...
                if (principal == null || (!isOrigin && !isDestination && !isAdmin)) {
                        log.warn("[SECURITY] Unauthorized attempt to delete agreement ID: {} by user: {}", id,
                                        principal != null ? principal.getName() : "anonymous");
                        throw new ResponseStatusException(HttpStatus.FORBIDDEN,
                                        "You are not authorized to delete this agreement");
                }
                ...
                if (AgreementStatus.COMPLETED.equals(agreement.getStatus())) {
                        throw new ResponseStatusException(
                                        ...
                                        "Cannot delete an agreement that is already completed.");
                }
